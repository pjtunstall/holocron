...
fn test_holocron() {
    ...
    let mut guard = ConditionalCleanup::new(username);
    std::env::set_current_dir(std::env::current_dir().unwrap()).unwrap();
    let (kyber_ek, kyber_dk, rsa_ek, rsa_dk) = keys::generate_keys(&guard.username).unwrap();
    guard.activate(); // Only allow keys to be deleted if they were generated by the test.

    let public_key_path = format!("keys/{}_public_key.asc", &guard.username);

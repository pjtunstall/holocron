...
If online banking, commerce, etc. are to survive in something like their current form, they'll need new methods of encryption. Several have been proposed. Some candidates have proved ineffective even against classical computers, but a few possibilities remain. At the time of writing (late 2024), these potentially quantum-proof algorithms are just starting to be adopted. It's likely that, in the near future, hybrid systems will become common. [Signal](https://signal.org/docs/specifications/pqxdh), [Apple](https://security.apple.com/blog/imessage-pq3/), [Cloudflare](https://blog.cloudflare.com/post-quantum-to-origins), [AWS](https://www.amazon.science/blog/preparing-today-for-a-post-quantum-cryptographic-future), [Firefox](https://www.mozilla.org/en-US/firefox/135.0/releasenotes/), and the latest [Chrome](https://blog.chromium.org/2024/05/advancing-our-amazing-bet-on-asymmetric.html) desktop versions have all recently implemented encryption systems that combine well-establish classical algorithms with a hopefully quantum-proof component. Post-quantum encryption/decryption is more widely adopted than post-quantum digital signatures, being considered a higher priority as well as easier to implement.[^3]
...
[^3]: UPDATE: Chrome and Firefox both use the hybrid algorithm X25519MLKEM7 by default now. In Firefox, it can be toggled; to check that it's enabled, enter `about:config` into the search bar, accept risk and continue, then enter `security.tls.enable_kyber` and confirm that it's set to true; likewise check that `network.http.http3.enable_kyber` is set to true. You can also use Bas Westerbaan's [PQSpy](https://addons.mozilla.org/en-GB/firefox/addon/pqspy/) add-on to verify that post-quantum encryption is enabled. The [rustls crate now supports post-quantum cryptography](https://github.com/rustls/rustls/tree/main/rustls-post-quantum) by default. From Go 1.24, released 11th Febrary 2025, Go's standard library, via the [crypto/tls](https://pkg.go.dev/crypto/tls) package, includes the X25519MLKEM768 hybrid post-quantum key exchange for TLS by default. See [here](https://blog.cloudflare.com/pq-2024/) for a detailed summary of the state of affairs in March 2024, and [here](https://www.netmeister.org/blog/pqc-2025-02.html) for a shorter summary from February 2025. To test whether your browser supports post-quantum encryption, check [here](https://pq.cloudflareresearch.com/) or [here](https://isitquantumsafe.info/). [Node.js supports PQC out of the box via bundled OpenSSL](https://dev.to/daan_acohen/is-your-nodejs-app-quantum-safe-tracing-pqc-in-tls-connections-10b8). In March 2026, OpenPGP is working towards ratification of it's own specification for post-quantum cryptography, [draft-ietf-openpgp-pqc-17](https://datatracker.ietf.org/doc/draft-ietf-openpgp-pqc/). As of November 2025, this has already been adopted, partly, in pre-release versions by [Sequoia PGP](https://sequoia-pgp.org/blog/2025/11/15/202511-post-quantum-cryptography/).

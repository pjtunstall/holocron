...
[^1]: Even if supported, post-quantum cryptography may not be the default. For example, as of February 2025, Chrome and Firefox both use the hybrid algorithm X25519MLKEM768, but in Chrome it needs to be explicitly enabled with two flags, `chrome://flags/#enable-tls13-kyber` and `chrome://flags/#use-ml-kem`. In Firefox, enter `about:config` into the search bar, then select `security.tls.enable_kyber` and `network.http.http3.enable_kyber`; you can use Bas Westerbaan's [PQSpy](https://addons.mozilla.org/en-GB/firefox/addon/pqspy/) add-on to verify that post-quantum encryption is enabled.
